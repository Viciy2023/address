...
export const CARD: Record<SiteLang, CardStrings> = {
  zh: {
    ...
    partialHint: "用 x 或 * 代表待补全的位；只填前缀则自动补足长度。",
    complete: "补全卡号",
    resultsTitle: "生成的测试卡",
    cardNumber: "卡号",
    expiry: "有效期",
    ...
    guideSteps: [
      ...
      { title: "需要时就按片段补全", body: "如果手上已有部分卡号或已知 BIN，切到「片段补全」，把已知部分粘进去，用 x 或 * 占位，即可得到完整卡号。" },
  ...
  "zh-hant": {
    ...
    partialLabel: "卡號片段",
    partialPlaceholder: "例如 6222 或 4111xxxxxxxxxxxx",
    partialHint: "用 x 或 * 代表待補全的位；只填前綴會自動補足長度。",
    complete: "補全卡號",
    resultsTitle: "產生的測試卡",
    cardNumber: "卡號",
    expiry: "有效期限",
    ...
    guideSteps: [
      ...
      { title: "需要時用片段補全", body: "若手上已有部分卡號或已知 BIN，切到「片段補全」，把已知部分貼進去，用 x 或 * 佔位，即可得到完整卡號。" },
  ...
  en: {
    ...
    partialHint: "Use x or * for the digits to fill; a bare prefix is padded to full length.",
    complete: "Complete",
    ...
    guideSteps: [
      ...
      { title: "Complete a partial number", body: "If you already have part of a number or a known BIN, switch to Complete, paste what you have with x or * for the gaps, and get a full number back." },
  ...
  ja: {
    ...
    partialHint: "補完する桁は x か * で指定します。接頭辞だけの場合は全桁まで補います。",
    complete: "補完する",
    ...
    guideSteps: [
      ...
      { title: "必要なら一部から補完", body: "すでに番号の一部や BIN がある場合は「一部から補完」に切り替え、既知の部分を貼り付け、不明な桁を x か * にすれば完全な番号が得られます。" },
  ...
  ko: {
    ...
    partialHint: "채울 자리는 x 또는 * 로 표시하세요. 접두사만 입력하면 전체 길이로 채웁니다.",
    complete: "완성하기",
    ...
    guideSteps: [
      ...
      { title: "필요하면 일부로 완성", body: "이미 번호 일부나 BIN 이 있다면 ‘일부로 완성’으로 전환해 아는 부분을 붙여넣고 모르는 자리를 x 또는 * 로 두면 완전한 번호를 얻습니다." },
